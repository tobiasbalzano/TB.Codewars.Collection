...
namespace _7Kyu
{
    ...
    {
        //Target Date kata https://www.codewars.com/kata/target-date/train/csharp description:
        ...
        {
            DateTime insertDate;
            DateTime.TryParse("2016-01-01", out insertDate);
            DateTime reachedDate = insertDate;
            while (a0 < a)
            {
                reachedDate = reachedDate.AddDays(1);
                a0 += a0 * (p / 36000);
            }
            return reachedDate.ToString("yyyy-MM-dd");
        }
    }
}
